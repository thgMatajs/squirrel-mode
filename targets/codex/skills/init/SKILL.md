---
name: init
description: "Run the squirrel-mode calibration interview: seven one-at-a-time multiple-choice questions that build the user's personal response-formatting profile. Only run this when the user explicitly asks to run squirrel-mode calibration - for example, asked to run squirrel init or calibrate squirrel-mode. Never start this interview unprompted, and never run it merely because no profile exists yet."
---

<!-- GENERATED FILE. Source: skills/init/SKILL.md. Generator: scripts/build.sh.
     Hand edits to this file will be overwritten the next time scripts/build.sh runs. -->

# squirrel-mode calibration (Codex)
...
Sets: `max_list_items` (3, 5, or 7). A free-form or typed-in answer is accepted only if it is a whole number from 3 to 7 inclusive - the same range the defaults table in `~/.codex/AGENTS.md` and the squirrel-mode `tune` skill both enforce for this field. A number outside that range, or anything that is not a number, is not accepted: say in one line that it must be an integer from 3 to 7, and ask this same question again, still showing `Question 5 of 7`. Do not move on until a valid value is given.
...
6. Once the file is written, confirm in one line and immediately answer the user's very next message using the new profile. The demonstration is part of this skill itself, not a separate step the user has to ask for.
