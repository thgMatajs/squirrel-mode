---
name: squirrel-pickup
description: "Resume work on this project after an interruption or at the start of a new session: show recent wins, what was being done, the next action, and open decisions, in that order, then stop. Trigger only on an explicit request to resume or pick up this project's past work at the start of a new session or after a stated interruption. Never trigger on a request to continue the live, current conversation (e.g. 'let's continue' or 'where were we' about what was just discussed in this same session), and never merely because a checkpoint exists."
disable-model-invocation: true
---

<!-- GENERATED FILE. Source: skills/pickup/SKILL.md. Generator: scripts/build.sh.
     Hand edits to this file will be overwritten the next time scripts/build.sh runs. -->

# squirrel-mode pickup (Cursor)

This skill shows what this project's checkpoint remembers, in a fixed order, then stops.
