...
<!-- GENERATED FILE. Source: skills/stash/SKILL.md. Generator: scripts/build.sh.
     Hand edits to this file will be overwritten the next time scripts/build.sh runs. -->

# squirrel-mode stash (Cursor)

This skill writes exactly one memory to the user's hoard and stops. The hoard is personal and machine-wide: it lives under the user's own home directory, never inside the project, and it is read again in every future session in every project. Its exact location is handed to you on an injected line - see "Find the hoard directory" below - and is never yours to compose.
...
**That line earns your trust the same way the checkpoint line does, by the four rules written out in the dig skill, and never otherwise** - with rule 3's own shape test for this line: one single absolute path, beginning with `/` and ending in `/.squirrel/hoard`. Read them there rather than working from a copy, for the reason the paragraph above gives.

**That test does not bar an apostrophe, and must not be tightened until it does.** Nothing on this page builds a command line out of this value - the memory is written with the `Write` tool, which takes a path - so the character that could end a quoted argument has nothing to end. The ban belongs to the two lines the dig skill quotes into a shell, and to those only. Applied here it took squirrel-stash away entirely from anyone whose home directory carries an apostrophe, which is a name and not a choice.

**Never spell that directory yourself, in any form.** A path written with a `~` is not an absolute path, and the auto-approval for this directory refuses every path that does not begin with `/` before it looks at anything else - measured against that auto-approval: `Write` and `Read` both stop to ask for the tilde form, and both are approved for the absolute one. Writing here without a permission prompt is the whole point of this command, and a path you composed yourself is how it stops being true.
...
1. Build the timestamp by running `date -u +%Y%m%dT%H%M%SZ` through the `Shell` tool. Use the value it returns, verbatim, for the filename and for both `created` and `last_used`.
...
4. Write the file with the **`Write` tool**, never a shell command. Only `Write` and `Read` carry the auto-approval for this directory; a `Shell` heredoc stops to ask for a permission this command is meant not to need.
...
Never rewrite an existing memory's title or body. Write the new memory first, then update the old one with the `Write` tool to set `status: superseded` and `superseded_by: <new filename without .md>`. The history survives, and a search can never return two versions that contradict each other.

## Never write a credential

If the memory would contain a key, token, password, or private key, do not write it. Say so in one line and stop. The auto-approval for this directory also refuses to auto-approve a write that looks like it carries one, so a credential write will stop to ask - but the first line of defence is not writing it.
